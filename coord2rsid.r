...
# Annot.dir = 'C:/Users/Eugene/Desktop/Annotation'
  ...
  for (i in 1:length(folders)) {
    ...
    if (!length(files) > 0) {
      ...
      for (j in 1:length(files)) {
        ...
        # multi-column, NO HEADER w/ empty 1st col
        } else if ( ncol(orig)>1 && !( ("SNP" %in% orig[1,]) || 
                                       ("snp" %in% orig[1,]) ) )  {
          
          orig <- read.table(files[j], header=FALSE)
          snp <- as.vector(orig$V2, mode="character")
